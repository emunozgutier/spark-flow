...
export const DebugBar: React.FC<DebugBarProps> = ({
  ...
  // Synchronize textarea state with live elements whenever the modal opens or elements change
  useEffect(() => {
    if (!isOpen) return;
    try {
      ...
      console.error('Failed to serialize elements for portal:', err);
    }
  }, [elements, isOpen]);
  ...
  return (
    <>
      ...
      {/* Sub-toolbar Trigger Button */}
      <div className="interactive-panel glass-panel topbar-sub animate-slide-down debug-bar-panel" style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '6px 12px' }}>
        <button
        ...
        >
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ stroke: 'var(--theme-coral)' }}>
            <rect x="3" y="3" width="18" height="18" rx="2" ry="2"/>
            <rect x="9" y="9" width="8" height="8"/>
            <line x1="17" y1="21" x2="21" y2="21"/>
            <line x1="21" y1="17" x2="21" y2="21"/>
          </svg>
          <span>Open State Portal (Copy & Paste)</span>
        </button>
      </div>

      {/* 80% Screen Width & Height Modal Popup Overlay */}
      {isOpen && (
        <div
